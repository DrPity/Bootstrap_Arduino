...
def bootstrap(directory):
    ...
    serial = options("Configure serial communication?" + BLUE + " [Yes/No]: " + NOCOLOR, validResponse, "[Yes/No - y/n]")
    if serial:
        baudrate = options("Enter Baudrate" + BLUE + " [9600,57600,115200]: " + NOCOLOR, validBaudrate, "[9600,57600,115200]")

    neoPixel = options("Import and set up NeoPixel library and wrapper?" + BLUE + " [Yes/No]: " + NOCOLOR, validResponse, "[Yes/No - y/n]")
    if neoPixel:
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description=GREEN + 'Bootstraps a C++ Arduino project for quick prototyping' + NOCOLOR)
